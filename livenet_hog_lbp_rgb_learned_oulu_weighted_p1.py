...
print(sys.path)
from  CNN_networks.hog_lbp_rgb_all_perturb import cnn_hybrid_color_single
...
def main(args):
    ...
    # balance the training data for binary class classification
     ...
     # first balance the training data
    ...
    while batch_num < args.max_epochs:
        ...
        for i in range(train_generator_r.n // train_generator_r.batch_size):
            ...
            for j in range(x_r.shape[0]):
                ...
                lbp_image_data = cv2.cvtColor(x_r[j], cv2.COLOR_RGB2GRAY)
                print(lbp_data)
                cv2.imshow('img', np.asarray(lbp_image_data))
                cv2.waitKey()

                # compute the histogram of oriented gradients
                _, hog_im_data = hog(lbp_image_data, orientations=8, pixels_per_cell=(16, 16),
                                     cells_per_block=(1, 1), visualise=True)

                hog_data.append(hog_im_data)


                # compute the local binary pattern

                lbp_im_data = lbp(image=lbp_image_data, P=args.lbp_pts, R=args.lbp_r, method='uniform')
                lbp_data.append(lbp_im_data)

            x_l = np.expand_dims(np.asarray(lbp_data), axis=-1)
            x_h = np.expand_dims(np.asarray(hog_data), axis=-1)


            data_final = np.concatenate((x_h, x_l), axis=-1)


            x_r = x_r.astype('float32') / 255

            tr_acc1 = model_final.fit([x_r, data_final],
                                      y,
        ...
        for i in range(test_generator_r.n // test_generator_r.batch_size):
            ...
            hog_data = []
            for j in range(x_r.shape[0]):
                lbp_image_data = cv2.cvtColor(np.asarray(x_r[j]), cv2.COLOR_RGB2GRAY)
            ...
            tst_loss = model_final.evaluate([x_r, data_final],
                                            y,
                                            verbose=0)
        ...
        if test_acc > accuracy:
            model_final.save_weights(
                ...
                'learned_hog_lbp_rgb_all_perturb_protocol1_'
                + str(args.lbp_pts) + '_' + str (args.lbp_r) + '_' + '5x5_160x160_hog_online_lbp_lev1_' + str(args.max_epochs) + '.h5')
            accuracy = test_acc
            print('saved')
