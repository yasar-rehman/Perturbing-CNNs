...
def video_data(filepath, frame_length):
    ...
    while batch < frame_length:
        ...
        if (len(faces) !=0) and (len(faces) == 1) :
           # crop the face based on the eyes position
            for f in faces:
                # (x, y, w, h) = np.abs(rect_to_bb(f))
                shape = predictor(miniframe, f)
                shape = shape_to_np(shape)
                remapped_image = cv2.convexHull(shape)
                rect = cv2.boundingRect(remapped_image)
                x = rect[0]
                y = rect[1]
                w = rect[2]
                h = rect[3]
                im_size = np.int(np.sqrt(w * h))
                out=cv2.resize(selected_frames[y+10:y + h-30, x+20:x + w-20 ], (im_size,im_size),cv2.INTER_CUBIC)
                cv2.imshow('img', out)
...
def get_dataset(paths,filename): #this function get the dataset from the given path
    ...
    if database[-1] in {'Train.txt'} and st_file_path[-1] in {'train'} :
        database_path = os.path.expanduser('~/Documents/Newwork/OULU_NPU/Train_files/')

    if database[-1] in {'Dev.txt'} and st_file_path[-1] in {'dev'}:
        database_path = os.path.expanduser('~/Documents/Newwork/OULU_NPU/Dev_files/')

    if database[-1] in {'Test.txt'} and st_file_path[-1] in {'test'}:
...
#
# path1 = os.path.expanduser('~/Documents/Newwork/OULU_NPU/Protocols/Protocol_1/Train.txt')
# path2 = os.path.expanduser('~/Documents/Newwork/OULU_NPU/Protocols/Protocol_1/Dev.txt')
# path3 = os.path.expanduser('~/Documents/Newwork/OULU_NPU/Protocols/Protocol_1/Test.txt')
#
#
#
# # first make a list of train and test text files to store the corresponding paths and labels
#
# train_filepath = os.path.expanduser('~/Documents/Newwork/OULU_protocol1_avg/train_avg') # file path for training
# dev_filepath = os.path.expanduser('~/Documents/Newwork/OULU_protocol1_avg/dev_avg')
# test_filepath = os.path.expanduser('~/Documents/Newwork/OULU_protocol1_avg/test_avg')
#
...
# #
# #
# parent_path = '~/Documents/Newwork/OULU_FACE_Protocol1_avg'
# # #
# child_path1 = 'train_avg'
# child_path2 = 'dev_avg'
# child_path3 = 'test_avg'
# # #
# textfile1 = '~/Documents/Newwork/OULU_FACE_Protocol1_avg'
# # #
